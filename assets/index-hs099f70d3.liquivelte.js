  ...
  Array.from(document.querySelectorAll('.liquivelte-component.exploding-gallery')).forEach(wrapper => {
    ...
    onIntersect(wrapper, ([entry]) => {
      (async () => {
        if(entry.isIntersecting && !initialized) {
          initialized = true;
          wrapper.svelteComponent = new (await import('./exploding-gallery-hs39353e1f.liquivelte.js')).default({
            target: wrapper,
            hydrate: true,
            props: {
                ...svelteProps,
  ...
  Array.from(document.querySelectorAll('.liquivelte-component.scroll-animation')).forEach(wrapper => {
    ...
    onIntersect(wrapper, ([entry]) => {
      (async () => {
        if(entry.isIntersecting && !initialized) {
          initialized = true;
          wrapper.svelteComponent = new (await import('./scroll-animation-hs240075cc.liquivelte.js')).default({
            target: wrapper,
            hydrate: true,
            props: {
                ...svelteProps,
