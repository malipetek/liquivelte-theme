const onIntersect = (el, callback) => {
  ...
  Array.from(document.querySelectorAll('.liquivelte-component.main-cart')).forEach(wrapper => {
    ...
    onIntersect(wrapper, ([entry]) => {
      (async () => {
        if(entry.isIntersecting && !initialized) {
          initialized = true;
          wrapper.svelteComponent = new (await import('./main-cart-hs61ec4cb7.liquivelte.js')).default({
            target: wrapper,
            hydrate: true,
            props: {
                ...svelteProps,
