    ...
    onIntersect(wrapper, ([entry]) => {
      (async () => {
        if(entry.isIntersecting && !initialized) {
          initialized = true;
          wrapper.svelteComponent = new (await import('./main-page-hsfb5de13a.liquivelte.js')).default({
            target: wrapper,
            hydrate: true,
  ...
  Array.from(doc.querySelectorAll('.liquivelte-component.contact-form')).forEach(wrapper => {
    ...
    onIntersect(wrapper, ([entry]) => {
      (async () => {
        if(entry.isIntersecting && !initialized) {
          initialized = true;
          wrapper.svelteComponent = new (await import('./contact-form-hse593155e.liquivelte.js')).default({
            target: wrapper,
            hydrate: true,
            props: { resetCicR: true },
            context: new Map([['svelteProps', svelteProps], ['rawIncludes', rawIncludes], ['lec', liquid_expression_cache]])
