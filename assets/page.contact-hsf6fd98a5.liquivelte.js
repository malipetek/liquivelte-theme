    ...
    onIntersect(wrapper, ([entry]) => {
      (async () => {
        if(entry.isIntersecting && !initialized) {
          initialized = true;
          wrapper.svelteComponent = new (await import('./main-page-hs8720894f.liquivelte.js')).default({
            target: wrapper,
            hydrate: true,
  ...
  Array.from(doc.querySelectorAll('.liquivelte-component.contact-form')).forEach(wrapper => {
    ...
    onIntersect(wrapper, ([entry]) => {
      (async () => {
        if(entry.isIntersecting && !initialized) {
          initialized = true;
          wrapper.svelteComponent = new (await import('./contact-form-hsdec9aa46.liquivelte.js')).default({
            target: wrapper,
            hydrate: true,
            props: { resetCicR: true },
            context: new Map([['svelteProps', svelteProps], ['rawIncludes', rawIncludes], ['lec', liquid_expression_cache]])
