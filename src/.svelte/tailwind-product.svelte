...
export let form_inputs_fWNCLaw;
export let form_props_fWNCLaw;
...
{@const forloop = {
  ...
  rindex: ( breadcrumbs).length - index,
  rindex0: ( breadcrumbs).length - index - 1,
  length: ( breadcrumbs).length,
} }
          {#if !(index == breadcrumbs_size )}
            {#if !(item == '' )}
              <li> 
                <div class="flex items-center">
                  <a href="{ liquid.prepend(item, '/') }" class="mr-2 text-sm font-medium text-gray-900"> { liquid.capitalize(item) } </a>
  ...
  length: ( product.images).length,
    ...
    <div class="max-w-2xl mx-auto pt-10 pb-16 px-4 sm:px-6 lg:max-w-7xl lg:pt-16 lg:pb-24 lg:px-8 lg:grid lg:grid-cols-3 lg:grid-rows-[auto,auto,1fr] lg:gap-x-8">
      ...
      <div class="mt-4 lg:mt-0 lg:row-span-3">
        ...
        <form   class="mt-10"  {...form_props_fWNCLaw[index || 0]}>
  {@html form_inputs_fWNCLaw[index || 0]}
